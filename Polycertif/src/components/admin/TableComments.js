...
export default function TableComments() {
  ...
  const commentsEl = currentComment.map((comment) => (
    <tr key={comment.id_comment}>
        ...
        <div className="flex items-center">
          <div className="flex-shrink-0">
            <Link to="#" className="block relative">
              <img
                alt="profile"
                src="https://img1.freepng.fr/20180320/bwe/kisspng-computer-icons-icon-design-iconfinder-desktop-wall-vectors-free-download-icon-comment-5ab12ed2df5c89.5744351015215612989149.jpg"
      ...
      <td className="px-5 py-3 border-b border-gray-200 bg-white text-sm">
        <p className="text-gray-900 whitespace-no-wrap">{comment.comment_time }</p>
      </td>
      <td className="px-5 py-3 border-b border-gray-200 bg-white text-sm ">
        <form onSubmit={() => deleted(comment.id_comment)}>
        <Link to={`/back/Comments/${comment.id_comment}`}>
